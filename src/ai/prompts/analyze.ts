...
- 【关键】仔细区分：用户是要"凭空创造不存在的内容"(task_type=create_new)还是"从已有文档搬运信息"(task_type=extract_and_fill)
- 如果用户说"编一个""新建""起一个"等词 → 创建新内容，后续计划中不要从已有文档提取具体数据
...
- 如果用户提供模板文档+空白表，为模板中的每个可填充字段生成独立 operation（填表场景）
